...
<form action="${pageContext.request.contextPath}/user" method="post">
    ...
    <c:if test="${sessionScope.loggedUser.userStateId == 4 and sessionScope.loggedUser.userId != requestScope.user.id}">
        ...
        </select> <button class="btn-class" type="submit" name="idChangeState" value="${requestScope.user.id}">Сменить статус пользователя</button></p>
    </c:if>
    <c:if test="${sessionScope.loggedUser.userId == requestScope.user.id}">
        <button class="btn-class" type="submit" name="idPassword" value="${requestScope.user.id}">Сменить пароль</button></p>
    </c:if>
